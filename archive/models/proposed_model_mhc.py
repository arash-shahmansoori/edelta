"""
mHC-Only Model: Manifold Hyper-Connection Mixing WITHOUT Geodesic Rotation.

This isolates the mHC mixing matrices to test if they alone provide benefit.
Comparison baseline for understanding which component helps.
"""
...
class BlockMHC(nn.Module):
    """
    Transformer block with mHC Mixing Matrices ONLY (no Geodesic rotation).
    
    This tests if the mixing matrices alone provide benefit over baseline.
    """
    def __init__(self, config):
        super().__init__()
        ...
        # These mix the input/output of attention and MLP
        ...
        # Identity initialization - start as standard transformer
        with torch.no_grad():
...
class GPTMHC(nn.Module):
    """GPT with mHC Mixing Matrices only (no Geodesic rotation)."""
    ...
    def __init__(self, config):
        ...
        self.transformer = nn.ModuleDict(dict(
            ...
            h=nn.ModuleList([BlockMHC(config) for _ in range(config.n_layer)]),
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type):
        """Standard optimizer without special geodesic handling."""
        decay_params = []
        nodecay_params = []
